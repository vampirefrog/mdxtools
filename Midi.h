...
public:
	bool dumpBytes;
	int headerLen, fileFormat, numTracks, ticksPerQuarterNote;
	...
	void readHeader() {
		if(!s.readCompare("MThd")) {
			throw exceptionf("not a MIDI file");
		}
		headerLen = s.readBigUint32();
	...
	void readTrack(int i) {
		if(!s.readCompare("MTrk")) {
			throw exceptionf("invalid track header at %p", s.tell());
		}
...
public:
	...
	virtual void handleUnknownCommand(uint8_t b) {}
...
private:
	virtual void handleHeader() {
		printf("MIDI headerLen=%d fileFormat=%d numTracks=%d ticksPerQuarterNote=%d\n", headerLen, fileFormat, numTracks, ticksPerQuarterNote);
	}
