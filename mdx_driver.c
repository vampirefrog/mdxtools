...
static void mdx_driver_note_on(struct mdx_driver *r, int track_num) {
	...
	if(track_num < 8) {
		...
		adpcm_driver_play(r->adpcm_driver, track_num - 8, r->pdx_file->samples[track->note].data, r->pdx_file->samples[track->note].len, track->adpcm_freq_num, mdx_adpcm_volume_from_opm(track->opm_volume + r->fade_value), track->note);
}
