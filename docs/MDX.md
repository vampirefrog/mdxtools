...
* `0x80-0xdf` `n` • Note data, followed by duration `n` + 1. Corresponds to MML command **n#,#**. `0x80` corresponds to MML **o0d+**, which means octave 0, note D♯. `0xdf` corresponds to **o8d**, octave 8, D. To calculate the MIDI equivalent note, subtract `0x80` and add `3`. For the PCM channels, P through W, this represents the sample index, the first sample is `0x80`, second sample is `0x81` and so on.
