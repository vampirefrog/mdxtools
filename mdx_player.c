...
static void mdx_player_set_tempo(struct mdx_driver *d, int tempo, void *data_ptr) {
	struct mdx_player *p = (struct mdx_player *)data_ptr;
	timer_set_opm_tempo(&p->opm_timer, OPM_CLOCK, p->sample_rate, tempo);
}

// static void mdx_player_set_adpcm_freq(struct adpcm_driver *d, int clock, int divisor, void *data_ptr) {
// 	struct mdx_player *p = (struct mdx_player *)data_ptr;
...
void mdx_player_start(struct mdx_player *p, struct mdx_file *f, int sample_rate) {
	...
	p->driver.data_ptr = p;
	p->driver.set_tempo = mdx_player_set_tempo;
	// p->driver.adpcm_driver.rate_changed = mdx_player_set_adpcm_freq;
	p->pcm_enable = 1;
	mdx_driver_init(&p->driver, f);
}

void mdx_player_end(struct mdx_player *p) {
}
