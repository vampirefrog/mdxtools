...
static void mdx_pcm_renderer_write_wait(struct mdx_pcm_driver *p, int wait, void *data) {
	struct mdx_pcm_renderer *r = (struct mdx_pcm_renderer *)data;
...
static void mdx_pcm_renderer_write_opm(struct mdx_driver *d, uint8_t reg, uint8_t val, void *data_ptr) {
	struct mdx_pcm_renderer *r = (struct mdx_pcm_renderer *)data_ptr;
	ym2151_write_reg(&r->opm, reg, val);
}

// static void mdx_pcm_renderer_write_oki(struct adpcm_driver *d, uint8_t port, uint8_t data, void *data_ptr) {
// //	struct mdx_pcm_renderer *r = (struct mdx_pcm_renderer *)data_ptr;
// 	// printf("oki write 0x%02x 0x%02x\n", port, data);
// 	okim6258_write(0, port, data);
// }

// static void mdx_pcm_renderer_set_adpcm_freq(struct mdx_pcm_driver *p, int clock, int divisor, void *data) {
// 	// printf("mdx_pcm_renderer_set_adpcm_freq clock=%d divisor=%d\n", clock, divisor);
// 	struct mdx_pcm_renderer *r = (struct mdx_pcm_renderer *)data;
...
void mdx_pcm_renderer_init(
	struct mdx_pcm_renderer *r, struct mdx_file *f, struct adpcm_driver *d,
	...
	r->player.data_ptr = r;
	r->player.write_wait = mdx_pcm_renderer_write_wait;
	r->player.driver.write_opm = mdx_pcm_renderer_write_opm;
	r->player.pcm_enable = 1;
	r->player.data_ptr = r;
	r->adpcm_driver = d;
	r->player.driver.adpcm_mixer = &d->mixer;

	mdx_pcm_driver_start(&r->player, f, sample_rate);
}

void mdx_pcm_renderer_render(struct mdx_pcm_renderer *r) {
	r->cur_sample = 0;
	mdx_pcm_driver_play(&r->player, r->num_buf_samples);
}
